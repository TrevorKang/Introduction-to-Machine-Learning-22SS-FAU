...
def autoContrastSat(im, sat=0.004):
    ...
    for i in range(len(h_im)):
        ...
        if acc >= sat_hi:
            ...
            break
    print
    ("Total image pixel count: %d" % size_im)
    print
    "lower saturation value:  %d, at intensity: %d" % (sat_lo, i_lo)
    print
    "upper saturation value:  %d, at intensity: %d" % (sat_hi, i_hi)
...
def makeCLhist(im, maxslope=3.5, maxiter=10, redistribute=False, verbose=True):
    ...
    if verbose:
        print
        "Building slope-limited histogram, target slope =", maxslope,
        print
        "scaled as maxH =", maxH
    im = im.ravel()
...
def makeCLchist(im, maxslope=3.5, maxiter=10, redistribute=False, verbose=False):
    ...
    if verbose: print
    "Histogram sum =", hs.sum()
    chs = np.cumsum(hs)
    if verbose: print
    "Cumulative histogram max =", chs.max()
